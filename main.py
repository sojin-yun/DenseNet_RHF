...
def drive(args) :
    ...
    if not flags['baseline'] :
        model = ResNet_ensemble_deeper(block = BasicBlock_ensemble_deeper, layers = [3, 4, 6, 3], boundary_layers = [128, 256, 512], num_classes = 100, device = device)
    else :
        model = ResNet_deeper(block = BasicBlock_deeper, layers = [3, 4, 6, 3], num_classes = 100)
